...
/**
 * CRC-16 used by the AMPA Tablet Communication Protocol (RevG, section 1.3.5).
 *
 * Table-driven, MSB-first, polynomial 0x8005, initial remainder 0x0000,
...
final class Crc16 {
    ...
    /**
     * Return a new array equal to {@code data} with the 2-byte CRC appended
     * as [CRC MSB, CRC LSB] (the wire order the protocol expects).
     */
    static byte[] append(byte[] data) {
        int crc = compute(data, 0, data.length);
        byte[] out = new byte[data.length + 2];
        System.arraycopy(data, 0, out, 0, data.length);
        out[data.length] = (byte) ((crc >> 8) & 0xFF);
        out[data.length + 1] = (byte) (crc & 0xFF);
        return out;
    }

    /** Sanity check that the table/algorithm produce the documented check value. */
    static boolean selfTestPasses() {
        byte[] check = {'1', '2', '3', '4', '5', '6', '7', '8', '9'};
        return compute(check, 0, check.length) == CHECK_VALUE;
    }
}
